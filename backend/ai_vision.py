...
env = utils.read_env()
service = sdk.VisionServiceOptions(key=env['ai_vision_key'],
...
def get_vectorize_image(image_path):
    ...
    with open(image_path, 'rb') as f:
        image_data = f.read()
    url = (
        f'{env["vision_endpoint"]}computervision/retrieval:vectorizeImage?api-version=2023-02-01'
...
def get_vectorize_text(text):
    ...
    """
    url = (
        f'{env["vision_endpoint"]}computervision/retrieval:vectorizeText?api-version=2023-02-01'
        f'-preview&modelVersion=latest')
